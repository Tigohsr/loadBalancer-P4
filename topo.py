...
def main():
    ...
    sw_addr = ["10.0.%d.254" % n for n in xrange(nb_hosts)]
    ...
    s = net.get('s2')
    sw_mac_s2_eth2 = s.intf("s2-eth2").MAC()
    h = net.get('h2')
    h.setARP(sw_addr[1], sw_mac_s2_eth2)
    h.setDefaultRoute("dev eth0 via %s" % sw_addr[1])

    s = net.get('s3')
    sw_mac_s3_eth2 = s.intf("s3-eth2").MAC()
    h = net.get('h3')
    h.setARP(sw_addr[2], sw_mac_s3_eth2)
    h.setDefaultRoute("dev eth0 via %s" % sw_addr[2])

    s = net.get('s4')
    sw_mac_s4_eth4 = s.intf("s4-eth4").MAC()  
    h = net.get('h4')
    h.setARP(sw_addr[3], sw_mac_s4_eth4)
    ...
    for i in xrange(nb_switches):
        ...
        print "**********"
        if (i == 0):
		print "Running lb_s1.txt"
		cmd = [args.cli, "--json", args.json,
		       "--thrift-port", str(_THRIFT_BASE_PORT + i)]
		with open("lb_s1.txt", "r") as f:
		    print " ".join(cmd)
		    try:
			output = subprocess.check_output(cmd, stdin = f)
			#print ("Debugging switch match-action table")
